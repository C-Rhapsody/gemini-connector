...
var defaultMessages = Messages{
	...
	ErrorMediaNotSupported: "⚠️ 현재 지원하는 첨부 형식은 이미지와 다음 문서입니다: .txt, .md, .html, .pdf, .doc(x), .rtf, .odt, .csv, .xls(x), .ppt(x), .hwp(x). 동영상·음성·그 외 파일 형식은 지원하지 않습니다.",
	ErrorMediaDownloadFail: "첨부파일 다운로드에 실패했습니다.",
